...
// level control
var levelcont	: int = 1;
public static var speedup : int = 1;
public static var redinline : int = 0;
...
var inlock : int = 1; // in case of fly away, direction change can only happen once on each wall
...
function Start(){
	 ...
	 if(initx!=0){
	 	initdirection = initx/Mathf.Abs(initx);
	 }else{
	 	initdirection = inity/Mathf.Abs(inity);
	 }
...
function Update () {
	...
	if(x<0.14&&y<0.14){
		...
		redinline-=bullettype;
		if(redinline<0) redinline = 0;
